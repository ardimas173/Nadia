...
<div id="app">
	...
	<div class="box-info box box-solid view-item col-xs-12 col-lg-12 no-padding">
		...
		<div class="box-body no-padding">
			...
			<div class="col-md-4">
				<?= $form->field($FccModel, 'fees_collect_course_id')->dropDownList(ArrayHelper::map(Courses::find()->where(['is_status'=>0])->all(), 'course_id','course_name'),
					[
						'prompt'=>Yii::t('fees', 'Select Course'),
						'v-model'=>'courseid',
						'onchange'=>'
						$.get("'.Url::toRoute('dependent/get-course-section').'", { id: $(this).val() } )
						.done(function( data ) {
						$( "#'.Html::getInputId($FccModel, 'fees_collect_section_id').'" ).html( data );
						}
						);'
					]);
			...
			<div class="col-md-4">
				<?= $form->field($FccModel, 'fees_collect_section_id')->dropDownList(ArrayHelper::map(FeesCollectCategory::find()->where([
					'is_status'=>0,
					'fees_collect_course_id'=>$FccModel->fees_collect_course_id
				])->all(), 'fees_collect_section_id','section.section_name'),
					[
						'prompt'=>Yii::t('fees', 'Select Course'),
						'v-model'=>'sectionid',
						'@change'=>'coursechange(courseid,sectionid)'
					]);
				$fccData = [];
				if(!empty($FccModel->fees_collect_section_id)) {
					$fccData = ArrayHelper::map(FeesCollectCategory::find()->where([
						'is_status'=>0,
						'fees_collect_course_id'=>$FccModel->fees_collect_course_id,
						'fees_collect_section_id'=>$FccModel->fees_collect_section_id
					])->all(), 'fees_collect_category_id', 'fees_collect_name');
				}
				?>
			</div>
	...
	if($dispStatus) :
		...
		$feesDetails = FeesCategoryDetails::find()->where(['fees_details_category_id' => $FccModel->fees_collect_category_id, 'is_status'=>0])->asArray()->all();

		$stuDataTmp = Yii::$app->db->createCommand("SELECT stu_master_id FROM stu_master 
				WHERE stu_master_batch_id=:fcbId 
				AND stu_master_course_id=:courseId 
				AND stu_master_section_id=:sectionId  
				AND is_status=:status")
			->bindValues([
				':fcbId' => $FccModel->fees_collect_batch_id,
				':courseId' => $FccModel->fees_collect_course_id,
				':sectionId' => $FccModel->fees_collect_section_id,
				':status' => 0,
			])
			->queryColumn();
		...
		$feesData = FeesPaymentTransaction::find()->where(['fees_pay_tran_batch_id' => $FccModel->fees_collect_batch_id, 'is_status'=>0])->asArray()->all();
...
<?php $a = Html::getInputId($FccModel, 'fees_collect_category_id'); ?>
...
<script>
	var app = new Vue({
		...
		methods:{
			coursechange:function(courseid,sectionid)
			{
				$.get("<?= Url::toRoute('/fees/dependent/get-collect-category') ?>?sectionid=" + sectionid + "&courseid=" + courseid )
					.done(function( data ) {
							$('#<?=$a?>').html(data);
